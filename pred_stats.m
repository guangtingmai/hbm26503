...
pathfolder = 'C:\Users\Guangting Mai\OneDrive - The University of Nottingham\LEL 2013';
addpath(genpath(pathfolder));

folder = sprintf('%s//analyses 2022',pathfolder);
filename = 'mTRF_encoding_Theta_RealPseudoBack_Phonemes(target)_SpectrogramPhonetics(partialed)2_0-500ms_eegraw_64HzFs';
...
r_real_topoplot = squeeze(mean(mean(r_real(:,:,:,1)),2));
r_pseudo_topoplot = squeeze(mean(mean(r_pseudo(:,:,:,1)),2));
r_back_topoplot = squeeze(mean(mean(r_back(:,:,:,1)),2));
...
r_real_stats = mean(mean(r_real(:,:,electrodes,1),2),3);
r_pseudo_stats = mean(mean(r_pseudo(:,:,electrodes,1),2),3);
r_back_stats = mean(mean(r_back(:,:,electrodes,1),2),3);

[mean(r_real_stats),std(r_real_stats)/(20^0.5)]
[mean(r_pseudo_stats),std(r_pseudo_stats)/(20^0.5)]
[mean(r_back_stats),std(r_back_stats)/(20^0.5)]

% r_real_shuffled_stats = mean(mean(r_real(:,:,electrodes,2),2),3);
% r_pseudo_shuffled_stats = mean(mean(r_pseudo(:,:,electrodes,2),2),3);
% r_back_shuffled_stats = mean(mean(r_back(:,:,electrodes,2),2),3);

% [~,p_real_vs_pseudo] = ttest(r_real_stats,r_pseudo_stats);
% [~,p_real_vs_back] = ttest(r_real_stats,r_back_stats);
% [~,p_pseudo_vs_back] = ttest(r_pseudo_stats,r_back_stats);
% p_real_vs_pseudo
% p_real_vs_back
% p_pseudo_vs_back
...
% p_real_vs_shuffled_bts = bootstrapping(r_real_stats-r_real_shuffled_stats,10000);
% p_pseudo_vs_shuffled_bts = bootstrapping(r_pseudo_stats-r_pseudo_shuffled_stats,10000);
%p_back_vs_shuffled_bts = bootstrapping(r_back_stats-r_back_shuffled_stats,10000);

%[~,~,~,adj_p]=fdr_bh([p_real_vs_pseudo,p_real_vs_back,p_pseudo_vs_back]);
...
% [mean(r_real_stats),mean(r_pseudo_stats);...
%     mean(r_real_shuffled_stats),mean(r_pseudo_shuffled_stats);...
%     p_real_vs_shuffled_bts,p_pseudo_vs_shuffled_bts]
